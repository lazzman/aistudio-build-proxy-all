...
let onStatusChangeCallback:
  | ((status: WebSocketProxyStatus, details?: string) => void)
  | null = null;
...
function updateStatus(newStatus: WebSocketProxyStatus, details?: string) {
  ...
  console.log(
    `WebSocket Proxy Status: ${currentStatus}${details ? ` - ${details}` : ""}`,
  );
}
...
function sendToServer(message: WSClientSentMessage) {
  if (socket && socket.readyState === WS_OPEN) {
    try {
      ...
      console.error(
        "WebSocket Proxy: Error serializing message for sending:",
        error,
        message,
      );
    }
    ...
    console.warn(
      "WebSocket Proxy: Cannot send message, socket not open.",
      message,
    );
  }
}
...
async function handleHttpRequest(request: WSHttpRequestMessage) {
  ...
  if (method === "GET") {
    try {
      ...
      if (
        parsedUrl.pathname.endsWith("/v1beta/models") ||
        parsedUrl.pathname.endsWith("/v1beta/models/")
      ) {
        if (parsedUrl.searchParams.has("key")) {
          ...
          console.log(
            `WebSocket Proxy: Modified URL for ${id} to remove 'key' param: ${url}`,
          );
        }
      }
    } catch (e) {
      console.error(
        `WebSocket Proxy: Error parsing URL for modification for request ID ${id}: ${url}`,
        e,
      );
    }
  }
  ...
  try {
    ...
    if (response.body && typeof response.body.getReader === "function") {
      ...
      console.log(
        `WebSocket Proxy: Sending http_response for request ID ${id}, status: ${response.status}, body length: ${responseBodyText.length}`,
      );
      sendToServer(httpResponseMessage);
    }
  } catch (error) {
    console.error(
      `WebSocket Proxy: Fetch error for request ID ${id} (${method} ${url}):`,
      error,
    );
...
function onSocketMessage(event: MessageEvent) {
  try {
    ...
    console.error(
      "WebSocket Proxy: Error parsing message from server or handling it:",
      error,
      event.data,
    );
  }
...
function onSocketClose(event: CloseEvent) {
  ...
  if (explicitClose) {
    updateStatus(
      WebSocketProxyStatus.IDLE,
      `Connection closed by client. Code: ${event.code}`,
    );
    explicitClose = false;
  } else {
    updateStatus(
      WebSocketProxyStatus.DISCONNECTED,
...
function scheduleReconnect() {
  if (explicitClose || !currentJwtToken) {
    updateStatus(
      WebSocketProxyStatus.IDLE,
      "Reconnection not attempted (explicit close or no token).",
    );
    return;
  }
  if (reconnectTimeoutId) {
  ...
  const delayWithJitter =
    currentReconnectDelay + Math.random() * RECONNECT_JITTER_MS;
  updateStatus(
    WebSocketProxyStatus.RECONNECTING,
    `Attempting to reconnect in ${Math.round(delayWithJitter / 1000)}s...`,
  );
  ...
  reconnectTimeoutId = window.setTimeout(() => {
    ...
    if (currentJwtToken) {
      ...
      updateStatus(
        WebSocketProxyStatus.IDLE,
        "Reconnect aborted: JWT token became unavailable.",
      );
    }
  }, delayWithJitter);
...
function connect(jwtToken: string) {
  if (!jwtToken) {
    // This check is still useful if connect is somehow called directly with a null/empty token
    updateStatus(
      WebSocketProxyStatus.ERROR,
      "JWT Token is required to connect.",
    );
    return;
  }
  ...
  try {
    ...
    updateStatus(
      WebSocketProxyStatus.ERROR,
      `Failed to instantiate WebSocket: ${error instanceof Error ? error.message : String(error)}`,
    );
    scheduleReconnect();
    return;
  }
...
function disconnect() {
  ...
  if (socket) {
    if (socket.readyState === WS_OPEN || socket.readyState === WS_CONNECTING) {
      ...
      onSocketClose({
        code: 1000,
        reason: "Client initiated disconnect on non-open socket",
        wasClean: true,
      } as CloseEvent);
    }
  } else {
...
function setOnStatusChange(
  callback: ((status: WebSocketProxyStatus, details?: string) => void) | null,
) {
